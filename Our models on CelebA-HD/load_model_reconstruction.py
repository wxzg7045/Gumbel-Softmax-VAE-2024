...
def compute_mse_ssim(original_images, reconstructed_images):
    mse_values = []
    ssim_values = []

    original_images = original_images.cpu().numpy().transpose(0, 2, 3, 1)
    reconstructed_images = reconstructed_images.cpu().detach().numpy().transpose(0, 2, 3, 1)
...
if __name__ == "__main__":
    ...
    best_model.tau_1 = nn.Parameter(torch.tensor(300.0).to(device))
    best_model.tau_2 = nn.Parameter(torch.tensor(300.0).to(device))
    best_model.eval()
    ...
    recon_batch = best_model(data)

    # Plot the original and reconstructed images
    plot_images(data, recon_batch)

    # Compute and print MSE and SSIM values
    mse, ssim_value = compute_mse_ssim(data, recon_batch)
