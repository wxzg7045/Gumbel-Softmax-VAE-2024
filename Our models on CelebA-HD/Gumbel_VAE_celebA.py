...
NUM_EPOCHS = 120
LOG_INTERVAL = 50
SAVE_PATH = r"F:\rPPGdata_4\CelebA\best_model_CelebA_desktop2070.pth"
...
def train_and_evaluate():
    ...
    lin_increase, lin_decrease = 1.0, 0.008
        ...
        for i in range(NUM_MODELS):
            if i != best_model_idx:
                if losses[i] > best_loss:
                    ...
                    # Reset tau values to their strategy-based values
                    if temp_strategies_extended[i] == 'fixed':
                        models[i].tau_1 = nn.Parameter(torch.tensor(float(initial_taus[i])), requires_grad=True)
                        models[i].tau_2 = nn.Parameter(torch.tensor(float(initial_taus[i])), requires_grad=True)
                    elif temp_strategies_extended[i] == 'linear_increase':
                        models[i].tau_1 = nn.Parameter(torch.tensor(float(min(initial_taus[i] + epoch * lin_increase, tau_max))), requires_grad=True)
                        models[i].tau_2 = nn.Parameter(torch.tensor(float(min(initial_taus[i] + epoch * lin_increase, tau_max))), requires_grad=True)
                    elif temp_strategies_extended[i] == 'linear_decrease':
                        models[i].tau_1 = nn.Parameter(torch.tensor(float(max(initial_taus[i] - epoch * lin_decrease, tau_min))), requires_grad=True)
                        models[i].tau_2 = nn.Parameter(torch.tensor(float(max(initial_taus[i] - epoch * lin_decrease, tau_min))), requires_grad=True)
                    elif temp_strategies_extended[i] == 'exp_increase':
                        models[i].tau_1 = nn.Parameter(torch.tensor(float(min(initial_taus[i] * (exp_increase ** epoch), tau_max))), requires_grad=True)
                        models[i].tau_2 = nn.Parameter(torch.tensor(float(min(initial_taus[i] * (exp_increase ** epoch), tau_max))), requires_grad=True)
                    elif temp_strategies_extended[i] == 'exp_decrease':
                        models[i].tau_1 = nn.Parameter(torch.tensor(float(max(initial_taus[i] * (exp_decrease ** epoch), tau_min))), requires_grad=True)
                        models[i].tau_2 = nn.Parameter(torch.tensor(float(max(initial_taus[i] * (exp_decrease ** epoch), tau_min))), requires_grad=True)
